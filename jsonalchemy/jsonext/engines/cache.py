...
class CacheStorage(Storage):
    ...
    def _set(self, data):
        ...
        self._cache.set(self._prefix + data['_id'], data, timeout=99999)

    def _get(self, id):
        value = self._cache.get(self._prefix + id)
        if value is None:
            raise KeyError()
        return value
    ...
    @property
    def _keys(self):
        return self._cache.get(self._prefix + '::keys') or set()

    @_keys.setter
    def _keys(self, value):
        self._cache.set(self._prefix + '::keys', value)

    def save_one(self, data, id=None):
        """See :meth:`~jsonalchemy.storage.Storage.save_one`."""
        if id is not None:
            data['_id'] = id
        self._set(data)
        return data
    ...
    def save_many(self, jsons, ids=None):
        """See :meth:`~jsonalchemy.storage.Storage.save_many`."""
        return map(lambda k: self.save_one(*k), zip(jsons, ids))

    def update_one(self, data, id=None):
        """See :meth:`~jsonalchemy.storage.Storage.update_one`."""
        if id is not None:
            data['_id'] = id
        id = data['_id']
    ...
    def update_many(self, jsons, ids=None):
        """See :meth:`~jsonalchemy.storage.Storage.update_many`."""
        return map(lambda k: self.update_one(*k), zip(jsons, ids))

    def get_one(self, id):
        """See :meth:`~jsonalchemy.storage.Storage.get_one`."""
        return self._get(id)

    def get_many(self, ids):
        """See :meth:`~jsonalchemy.storage.Storage.get_many`."""
    ...
    def get_field_values(self, ids, field, repetitive_values=True, count=False,
        ...
        """See :meth:`~jsonalchemy.storage.Storage.get_field_values`."""
        raise NotImplementedError()

    def get_fields_values(self, ids, fields, repetitive_values=True,
                          count=False, include_recid=False, split_by=0):
        """See :meth:`~jsonalchemy.storage.Storage.get_fields_values`."""
        raise NotImplementedError()

    def search(self, query):
        """See :meth:`~jsonalchemy.storage.Storage.search`."""
        def _find(item):
    ...
    def create(self):
        """See :meth:`~jsonalchemy.storage.Storage.create`."""
        assert len(self._keys) == 0

    def drop(self):
        """See :meth:`~jsonalchemy.storage.Storage.create`."""
        while self._keys:
            self._cache.delete(self._prefix + self._keys.pop())
