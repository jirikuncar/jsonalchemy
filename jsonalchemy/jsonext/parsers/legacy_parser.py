...
from jsonalchemy.parser import DecoratorOnEvalBaseExtensionParser
...
class LegacyParser(DecoratorOnEvalBaseExtensionParser):
    ...
    def create_element(cls, rule, field_def, content, metadata):
        ...
        """
        if getattr(metadata, '_legacy_field_matchings', None) is None:
            metadata._legacy_field_matchings = {}
        ...
        for legacy_rule in content:
            ...
            if inner_source_format not in metadata._legacy_field_matchings:
                metadata._legacy_field_matchings[inner_source_format] = {}
            ...
            for field_legacy_rule in legacy_rule:
                ...
                for legacy_field in legacy_fields:
                    if legacy_field not in metadata._legacy_field_matchings[
                            inner_source_format]:
                        metadata._legacy_field_matchings[inner_source_format][
                            legacy_field] = []
                    metadata._legacy_field_matchings[inner_source_format][
                        legacy_field].append(json_field)
